...
files_modified:
  ...
  - sdk/credentials/demo_credential.json
  - sdk/credentials/demo_credential_membership.json
  - sdk/index.html
autonomous: true
...
<task type="auto">
  ...
  <action>
    ...
    8. Copy demo credentials:
       - `cp scripts/demo_credential.json sdk/credentials/demo_credential.json`
       - `cp scripts/demo_credential_membership.json sdk/credentials/demo_credential_membership.json`
  ...
  <verify>
    ...
    - `ls sdk/credentials/` confirms demo credentials copied
...
<task type="auto">
  ...
  <action>
    ...
    NOTE on Vite public directory: Vite serves files from `public/` by default as static assets at the root URL. The credentials directory needs to be either in `sdk/public/credentials/` OR the vite.config.ts needs `publicDir` configured to make credentials accessible at `/credentials/`. The simplest approach: move credential files to `sdk/public/credentials/` OR configure `server: { fs: { allow: ['..'] } }` in Vite config. Choose the `public/` approach -- create `sdk/public/credentials/` and put demo credential JSONs there instead of `sdk/credentials/`.

    CORRECTION to Task 1 file locations: The demo credential JSONs should go to `sdk/public/credentials/` (not `sdk/credentials/`) so Vite serves them at `/credentials/` URLs. Update the copy commands accordingly.
  </action>
  <verify>
